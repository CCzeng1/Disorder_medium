...
didV=np.loadtxt(open("test_Rj_1.csv","rb"),delimiter=",",skiprows=0)
didV1=np.transpose(didV.reshape(501,201))

didV=np.loadtxt(open("test_Rj_2.csv","rb"),delimiter=",",skiprows=0)
didV2=np.transpose(didV.reshape(501,201))

didV=np.loadtxt(open("test_Rj.csv","rb"),delimiter=",",skiprows=0)
didV3=np.transpose(didV.reshape(501,201))

didV=np.loadtxt(open("test_RjV1_3.csv","rb"),delimiter=",",skiprows=0)
didV4=np.transpose(didV.reshape(501,201))

Bs=np.linspace(0,18,501)
mus=np.linspace(-3,6,201)

figsize=10,12
figure, [ax1,ax2,ax3,ax4] = plt.subplots(4,1,constrained_layout=True,figsize=figsize)
...
im3=ax4.pcolor(Bs,mus,didV4,cmap='PRGn',vmin=0.0, vmax=4,edgeColors='none')  # seismic  PRGn


cbar1=figure.colorbar(im1,ax=ax1,location='top',extend='max', spacing='proportional',
                      ticks=[0,2,4],pad=-0*10**(-2),aspect=45,shrink=0.9,) 
cbar1.ax.set_xticklabels(['0', '2', '4'])
cbar1.set_label(r'$G(\frac{e^2}{h})$', labelpad=0, x=0.72,rotation=0,fontsize=22)
...
# cbar1=figure.colorbar(im1,ax=ax1,location='top',extend='max', spacing='proportional',
#                       ticks=[0., 2, 4],pad=-2.5*10**(-2),aspect=15,shrink=0.9,) 
# cbar1.ax.set_xticklabels([ '0.', '2','4'])
# cbar1.set_label('$E_0(meV)$', labelpad=0, rotation=0,fontsize=22) # labelpad=-40, y=-0.02,

line,=ax1.plot(np.sqrt(mus**2+0.3**2),mus,color='#FF0000', linewidth=3,label=r'$V_{dis}=0$')
...
ax1.set_xlim([-0.0,18.0])
ax1.set_ylim([-3.0,6.0])

line,=ax2.plot(np.sqrt(mus**2+0.3**2),mus,color='#FF0000', linewidth=3,label=r'$V_{dis}=2.5 \times V_{imp3}$')
...
ax2.set_ylabel('$\mu (meV)$',family="Times New Roman" ) #,**csfont
ax2.set_xlim([-0.,18.])
ax2.set_ylim([-3.,6.])


line,=ax3.plot(np.sqrt(mus**2+0.3**2),mus,color='#FF0000', linewidth=3,label=r'$V_{dis}=5 \times V_{imp3}$')
...
# ax3.set_xlabel('$\Gamma (meV)$',family='Times New Roman') # ,**csfont
ax3.set_ylabel('$\mu (meV)$',family="Times New Roman" ) #,**csfont
ax3.set_xlim([-0.,18.])
ax3.set_ylim([-3.,6.])


line,=ax4.plot(np.sqrt(mus**2+0.3**2),mus,color='#FF0000', linewidth=3,label=r'$V_{dis}=5 \times V_{imp1}$')
ax4.legend(handles=[line],loc='lower left',bbox_to_anchor=(0,.0),
                  handlelength=0,handletextpad=0,fontsize=20)
ax4.set_xticks((0, 3, 6, 9, 12, 15, 18))
# ax3.set_xticklabels(['','','',''])
